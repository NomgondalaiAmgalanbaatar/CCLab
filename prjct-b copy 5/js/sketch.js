...
function draw() {
  ...
  if (countdownStarted) {
    timeRemaining = window.terminalSketch.getTimeRemaining();
    if (timeRemaining <= 0) {
      explodeBomb();
      return;
    }
...
function explodeBomb() {
  ...
  // full-screen white overlay div
  let whiteOverlay = createDiv('');
  ...
  whiteOverlay.style('background-color', 'white');
  whiteOverlay.style('z-index', '9999');
  whiteOverlay.style('opacity', '0');
  whiteOverlay.style('display', 'flex');
  whiteOverlay.style('justify-content', 'center');
  whiteOverlay.style('align-items', 'center');

  // "YOU FAILED" text
  let failedText = createDiv('YOU FAILED');
  failedText.style('font-family', 'Arial, sans-serif');
  failedText.style('font-size', '72px');
  failedText.style('color', 'red');
  failedText.style('text-align', 'center');

  whiteOverlay.child(failedText);
